...
$priceSchemes=($_REQUEST['priceSchemes']);
...
$newpath = Redirection::ICE_DOMAIN."/InstantWinGames/index.php";
...
?>
    <script>
        ...
        form_elem_html = '<form action="http://ala-new.winBetting.com/InstantGameEngineOLD/gamePlay/api/loadPortalGame.action" method="post" name="hidden_form" id="hidden_form">'+form_elem_html+'</form>';
...
else
{
    ...
    $playerBal = Utilities::getPlayerBalance();
   ...
   // $cashBalance = explode(".", $playerLoginResponse->walletBean->cashBalance)[0];
    ...
    $igePath= $newpath."?gameNum=".$gameNumb."&gameMode=".$gameMode."&domainName=tbg.lottoBetting.com&merchantKey=".$merchantKey."&secureKey=".$secureKey."&currency=".$currencyCode."&lang=".$lang."&gameType=".$gameType."&playerId=".$playerId."&merchantSessionId=".$merchantSessionId."&clientType=".$clientType."&deviceType=".$deviceType."&appType=".$appType."&userAgentIge=".$userAgentIge."&balance=".$cashBalance."&ticketPrice=".$ticketPrice."&priceSchemes=".urlencode($priceSchemes);
